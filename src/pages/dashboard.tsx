...
import { getRequests } from 'apiSdk/requests';
...
import { RequestListPage as ShowCaseList } from 'pages/requests';

function DashboardPage() {
  const { session } = useSession();

  const requestStat = useSWR<PaginatedInterface<RequestInterface>>(
    () => `/requests`,
    () => getRequests({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Requests',
      result: requestStat?.data?.totalCount,
